...
export default ({ mode }) => {
  ...
  return defineConfig({
    ...
    envPrefix: ['VITE_', 'TAURI_'], // 以 VITE_ TAURI_ 开头的环境变量会通过 import.meta.env 暴露在你的客户端源码中
    server: {
      host: '0.0.0.0', // 默认是 localhost
      port: 1420, // 设置开发端口（默认是 5173 端口）
      strictPort: true, // 设为 true 时若端口已被占用则会直接退出，而不是尝试下一个可用端口
      open: false, // 浏览器自动打开，设为字符串时，会被用作 URL 的路径名
      cors: true, // 为开发服务器配置 CORS，默认启用并允许任何源
      watch: {
        ignored: ['**/src-tauri/**'] // 系统监听器忽略 src-tauri
      },
      proxy: {
    ...
    plugins: [
      ...
      // 自动重启 vite 服务
      VitePluginRestart({
        restart: ['vite.config.ts', '.env', '.env.development', '.env.production']
      }),
      unpluginIcons({
        compiler: 'vue3',
